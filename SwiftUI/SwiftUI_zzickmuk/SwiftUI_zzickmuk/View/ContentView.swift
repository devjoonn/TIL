...
enum ViewOptions: Hashable {
    case homeFirst(ChampionModel)
    case homeSecond(ChampionModel)
        ...
        ScrollView {
            LazyVStack {
                ForEach(ChampionModel.mockChampions) { champion in
                    Button {
                        navPathFinder.addPath(option: .homeFirst(champion))
                    } label: {
                        HomeRowView(championModel: champion)
                    }
                    .tint(.primary)
                }
            }
...
struct HomeRowView: View {
    ...
    init(championModel: ChampionModel) {
...
struct HomeRowDestinationView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Button {
                navPathFinder.addPath(option: .homeSecond(champion))
            } label: {
                Text("스킨 사기")
                    .font(.title)
                    .bold()
...
struct HomeSkinBuyView: View {
    ...
    var body: some View {
        VStack(spacing: 30) {
            Text(champion.name).font(.largeTitle).bold() + Text("스킨 사시겠어요?")
